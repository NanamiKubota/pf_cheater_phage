...
pop5.plot <- ggplot() +
  geom_vline(xintercept = c(4345098,4355762), alpha=0.5, fill = "red")+
...
gc_stat2 <- ggplot(gc_long, aes(x = label, y = value,fill=label,color=label)) +
  ...
  stat_summary(fun.min=function(y)(mean(y)-sd(y)),
               ...
               geom="errorbar", width=0.5, color="black", linewidth=1) +
  stat_summary(fun=base::mean, geom="crossbar", fatten=3, width=0.3)+
  theme_bw()+
  theme(
    axis.title.y = element_blank(),
    axis.title.x = element_blank(),
...
```{r}
ggsave(plot=all_comp3, "./figures/net_fitness.png", height = 5, width = 14, device = "png", dpi = 300)
ggsave(plot=all_comp3, "./figures/net_fitness.svg", height = 5, width = 14, device = "png", dpi = 300)
```
...
```{r}
ggsave(plot=tp_plot_avg_no_anc, filename = "./figures/competition_timepoint_avg_WT.png", units = "cm", height = 18, width = 24, device = "png", dpi = 300) 

ggsave(plot=tp_plot_avg_no_anc, filename = "./figures/competition_timepoint_avg_WT.svg", units = "cm", height = 18, width = 24, device = "svg", dpi = 300)
```
...
twitch_plot <- ggplot(twitch, aes(x=Time_h, y=Twitch_mm, color=strain_l, fill=strain_l, group=strain_l))+
  geom_point(alpha=0.25, position=position_dodge(w = 0.5), size=3)+
  stat_summary(fun.min=function(y)(mean(y)-sd(y)),
               fun.max=function(y)(mean(y)+sd(y)),
               geom="errorbar", width = 0.4, position = position_dodge(w = 0.5),
               show.legend = FALSE, linewidth=1)+
  stat_summary(fun=base::mean, geom="crossbar", fatten = 4, width = 0.3, position = position_dodge(w = 0.5), show.legend = FALSE)+
...
```{r}
ggsave(plot = twitch_plot, "./figures/twitch.png", units = "cm", height = 15, width = 25, device = "png", dpi = 300)
ggsave(plot = twitch_plot, "./figures/twitch.svg", units = "cm", height = 15, width = 25, device = "svg", dpi = 300)
```
